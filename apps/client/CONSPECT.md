...
обработчики маршрутов позволяют создавать пользовательские обработчики запросов для заданного маршрута,
...
По умолчанию обработчики маршрутов кешируют запросы.
Существуют несколько способа отключения кеширования:
1) Экспортировать из route.ts константу dynamic (по умолчанию - "auto") (Segment Config Option).
2) Использовать объект Request с методом GET.
3) Использовать динамические функции, такие как headers() и cookie().
export const dynamic = "force-dynamic";
4) Использование любого HTTP-метода, отличного от GET.
...
Cookie - маленькие частички данных, которые сервер отправляет клиенту в ответе на запрос.
Браузер может сохранить файл cookie и отправить его обратно на сервер с последующими запросами.
Cookie в основном используются для трёх целей:
1) управление сеансами (например, login, хранение корзины покупок)
2) персонализация пользовательских настроек и тем
3) отслеживание (запись и анализ поведения пользователя)
...
Заголовки могут отправляться клиентом на сервер (Request Headers),
чтобы он мог правильно обработать полученные из запроса данные,
а также сервером клиенту (Response Headers) - содержат информацию о сервере и данных, направляемых в ответ на запрос.

Виды заголовков запроса:
'User-Agent' - идентифицирует браузер и ОС
'Accept' - указывает на тип контента (текст, видео, или изображения) и формат данных, которые клиент может обработать.
'Authorization' - используется клиентом для аутентификации себя на сервере.

Виды заголовков ответа:
'Content-Type' - сообщает клиенту тип данных возвращаемого контента (text/html, application/jsonб,...).
